...
U_07() {
    ...
    # 예외 계정(배포판 기본 계정): 로그인 쉘이 있어도 '불필요 계정'으로 보지 않음
    # 필요하면 여기에 추가 가능
    local except_users_regex='^(root|sync|shutdown|halt)$'

    # shadow 읽기 가능하면 "잠금 계정"도 로그인 불가로 간주 (정확도↑)
    local can_read_shadow=0
    [[ -r /etc/shadow ]] && can_read_shadow=1

    # helper: 계정이 잠금/로그인 불가 상태인지 판단
    #  - 패스워드 필드가 '!', '!!', '*', '*LK*' 등으로 시작하면 잠금으로 판단
    #  - 빈 값("")은 (환경마다) 패스워드 미설정일 수 있어 취약으로 볼 수도 있지만,
    #    여기서는 "쉘이 로그인 가능"이면 일단 의심 대상으로 남김.
    is_locked_shadow() {
        local u="$1" pw
        (( can_read_shadow == 0 )) && return 1
        pw="$(awk -F: -v uu="$u" '$1==uu{print $2; exit}' /etc/shadow 2>/dev/null)"
        [[ -z "$pw" ]] && return 1
        [[ "$pw" == '!'* || "$pw" == '*'* ]] && return 0
        return 1
    }

    # 시스템 계정(UID<UID_MIN) 중
    #  - 예외 계정 제외
    #  - (1) 로그인 차단 쉘이 아니고
    #  - (2) shadow 잠금도 아니면
    # => 취약 후보
    ...
    while IFS=: read -r user _ uid _ _ home shell; do
        ...
        # 예외 사용자 스킵
        [[ "$user" =~ $except_users_regex ]] && continue
        ...
        if (( uid < uid_min )); then
            ...
            # shadow에서 잠금이면 OK (읽을 수 있을 때만)
            if is_locked_shadow "$user"; then
                continue
            fi

            # 그 외는 의심
            suspicious+=("$user:$uid:$shell:$home")
        fi
    done < /etc/passwd
    ...
    if (( vuln == 0 )); then
        ...
        echo " (확인) UID_MIN=$uid_min, 예외계정=${except_users_regex}, shadow읽기=$can_read_shadow" >> "$resultfile" 2>&1
    else
        echo "※ U-07 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
        echo " 시스템 계정(UID<UID_MIN) 중 로그인 가능한 상태로 보이는 계정이 존재합니다." >> "$resultfile" 2>&1
        echo " (확인) UID_MIN=$uid_min, 예외계정=${except_users_regex}, shadow읽기=$can_read_shadow" >> "$resultfile" 2>&1
        echo " [의심 계정 목록] user:uid:shell:home" >> "$resultfile" 2>&1
        local x
        for x in "${suspicious[@]}"; do
            echo "  - $x" >> "$resultfile" 2>&1
        done
    fi
}
#연수
U_08() {
  echo ""  >> "$resultfile" 2>&1
  echo "▶ U-08(중) | 1. 계정관리 > 1.8 관리자 권한(그룹/ sudoers) 최소화 ◀"  >> "$resultfile" 2>&1
  ...
  # 특정 그룹의 구성원(/etc/group + /etc/gshadow 있으면) 수집
  ...
  # sudoers에서 사용자/그룹 토큰 추출(각 라인의 첫 토큰)
  ...
  _group_exists "sudo"  && admin_groups+=("sudo")

  # 그룹 구성원 중 root 외 계정 수집
  if [ "${#admin_groups[@]}" -gt 0 ]; then
    ...
    for g in "${admin_groups[@]}"; do
      while IFS= read -r u; do
        ...
        offenders+=("$u")
        evidence+="[관리자그룹:${g}] root 외 계정: ${u}\n"
      done < <(_collect_group_users "$g")
    done
  fi

  # -----------------------------
  # 2) sudoers 기반: root 외 sudo 권한 보유자 수집
  #  - 사용자 직접 등록: alice ALL=(ALL) ALL  -> alice
  #  - 그룹 등록: %devops ALL=(ALL) ALL -> devops 구성원 수집
  # -----------------------------
  local idtoken
    ...
    if echo "$idtoken" | grep -q '^%'; then
      ...
      if _group_exists "$sg"; then
        ...
        while IFS= read -r u; do
          ...
          _user_exists "$u" || continue
          offenders+=("$u")
          evidence+="[sudoers그룹:%${sg}] root 외 계정: ${u}\n"
  ...
  if [ "$vuln_found" -eq 1 ]; then
    ...
    echo " [관리자 계정 목록] ${offenders[*]}" >> "$resultfile" 2>&1
    echo -e " [근거]\n$evidence" >> "$resultfile" 2>&1
  else
    echo "※ U-08 결과 : 양호(Good)" >> "$resultfile" 2>&1
    echo " root 외 관리자 권한 계정이 확인되지 않았습니다." >> "$resultfile" 2>&1
  fi
...
U_17() {
    echo "" >> "$resultfile" 2>&1
    echo "▶ U-17(상) | 2. 파일 및 디렉터리 관리 > 2.4 시스템 시작 스크립트 권한 설정 ◀" >> "$resultfile" 2>&1
    ...
    if (( vuln == 0 )); then
        ...
        echo " 시스템 시작 스크립트/유닛 파일에서 root 미소유 또는 그룹/기타 쓰기 권한이 있는 항목이 존재합니다(예: ${offenders[0]})." >> "$resultfile" 2>&1
    fi
}
#연수
U_18() {
...
U_25() {
  ...
  # 하나라도 발견되면 바로 종료(-quit) → 빠름
  if samples="$(find / -xdev -type f -perm -0002 -print -quit 2>/dev/null)"; then
    if [ -n "$samples" ]; then
      found=1
    fi
  fi
  ...
  if [ "$found" -eq 1 ]; then
    ...
    echo " world writable 파일이 존재합니다." >> "$resultfile" 2>&1
    echo " [샘플] $samples" >> "$resultfile" 2>&1
    echo " (전체 목록은 아래 명령으로 확인) find / -xdev -type f -perm -0002 2>/dev/null" >> "$resultfile" 2>&1
  else
...
U_28() {
  ...
  echo " 양호 판단 기준 : (1) SSH/방화벽(nft/firewalld/iptables/sshd)에서 특정 IP/대역만 허용하는 제한 정책이 존재" >> "$resultfile" 2>&1

  # --- root 필요 (firewalld/nft/iptables 조회 권한) ---
  if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
    echo "※ U-28 결과 : N/A" >> "$resultfile" 2>&1
    echo " 사유: root 권한 필요(sudo로 실행해야 방화벽 정책 확인 가능)" >> "$resultfile" 2>&1
    return 0
  fi
  ...
  # ----------------------------------------
  # 1) SSHD 설정 기반 제한 정책
  #   - Match Address (IP/대역 조건)
  #   - (참고) AllowUsers/Groups 등은 "사용자 제한"이지 IP 제한은 아님
  # ----------------------------------------
  ...
  for f in $(_list_sshd_files); do
    ...
    if _norm_grep "$f" '^[[:space:]]*Match[[:space:]]+Address[[:space:]]+'; then
      good=1
      hit_type="SSHD"
      evidence+="[SSHD IP제한] ${f}에 Match Address(IP/대역 조건) 존재\n"
      break
    fi
  done
  ...
  # ----------------------------------------
  # 2) Firewalld 기반 제한
  #   - rich-rule: source address + (service ssh OR port 22)
  #   - zone sources 지정 + zone에서 ssh/22 허용
  # ----------------------------------------
  if [ "$good" -eq 0 ] && command -v firewall-cmd >/dev/null 2>&1; then
    if firewall-cmd --state >/dev/null 2>&1; then
      ...
      for z in $zones; do
        ...
        # source address + (service ssh OR port 22)
        if echo "$rr" | grep -Eqi 'source[[:space:]]+address=' && \
          ...
          good=1
          hit_type="FIREWALLD"
          evidence+="[FW IP제한] zone(${z}) rich-rule에 source+ssh/22 조건 존재\n"
          break
        fi

        # zone sources + 그 zone에 ssh/22 열림
        ...
        if [ -n "$srcs" ]; then
          ...
          if echo "$svc" | grep -qw ssh || echo "$ports" | grep -Eq '(^|[[:space:]])22/tcp([[:space:]]|$)'; then
            good=1
            hit_type="FIREWALLD"
            evidence+="[FW IP제한] zone(${z})에 sources(${srcs}) 지정 + SSH(22) 허용(소스 제한 형태)\n"
            break
          fi
        fi
      done
  ...
  # ----------------------------------------
  # 3) nftables 기반 제한(정확 탐지)
  #   - "input chain" 안에서
  #   - tcp dport 22 AND ip saddr/ip6 saddr AND (accept|drop|reject)
  #   - 표준 출력 형태가 달라도 잡히게 토큰 기반으로 체크
  # ----------------------------------------
  if [ "$good" -eq 0 ] && command -v nft >/dev/null 2>&1; then
    if nft list ruleset 2>/dev/null | awk '
      BEGIN {found=0; in_input=0}

      # chain input 블록 진입(다양한 출력 대응)
      $0 ~ /^[[:space:]]*chain[[:space:]]+input[[:space:]]*\{/ {in_input=1}

      # chain 블록 종료(대략적인 } 라인)
      in_input && $0 ~ /^[[:space:]]*}\s*$/ {in_input=0}

      # input chain 내부 규칙에서 조건 체크
      in_input {
        line=$0
        # 공통적으로 필요한 조건들
        has_dport22 = (line ~ /(tcp[[:space:]]+dport[[:space:]]+22|dport[[:space:]]+22|port[[:space:]]+22)/)
        has_saddr   = (line ~ /(ip6?[[:space:]]+saddr|ip[[:space:]]+saddr|ip6[[:space:]]+saddr|saddr[[:space:]]+)/)
        has_action  = (line ~ /(accept|drop|reject)/)

        if (has_dport22 && has_saddr && has_action) {
          found=1
          print line
        }
      }
      ...
      END { exit(found ? 0 : 1) }
    ' >/tmp/.u28_nft_hit 2>/dev/null; then
      good=1
      hit_type="NFT"
      local nft_sample
      nft_sample="$(head -n 3 /tmp/.u28_nft_hit 2>/dev/null | sed 's/^[[:space:]]*//')"
      evidence+="[NFT IP제한] input chain에서 SSH(22) source 제한 규칙 확인\n"
      [ -n "$nft_sample" ] && evidence+="[NFT 샘플]\n$(echo "$nft_sample" | sed 's/^/  - /')\n"
    fi
    rm -f /tmp/.u28_nft_hit 2>/dev/null
  fi

  # ----------------------------------------
  # 4) iptables 기반 제한(정확 탐지)
  #   - INPUT 체인에서 --dport 22 AND -s(소스) AND (ACCEPT|DROP|REJECT)
  # ----------------------------------------
  if [ "$good" -eq 0 ] && command -v iptables >/dev/null 2>&1; then
    if iptables -S INPUT 2>/dev/null | awk '
      BEGIN{found=0}
      {
        line=$0
        ...
        if (has_dport && has_src && has_act) {
          found=1
          print line
        }
      }
      END{exit(found?0:1)}
    ' >/tmp/.u28_ipt_hit 2>/dev/null; then
      good=1
      hit_type="IPTABLES"
      local ipt_sample
      ipt_sample="$(head -n 3 /tmp/.u28_ipt_hit 2>/dev/null)"
      evidence+="[IPTABLES IP제한] INPUT 체인에서 SSH(22) source 제한 규칙 확인\n"
      [ -n "$ipt_sample" ] && evidence+="[IPTABLES 샘플]\n$(echo "$ipt_sample" | sed 's/^/  - /')\n"
    fi
    rm -f /tmp/.u28_ipt_hit 2>/dev/null
  fi
  ...
  if [ "$good" -eq 1 ]; then
    ...
    echo " 접속 IP/대역 제한 정책이 확인되었습니다. (type=${hit_type})" >> "$resultfile" 2>&1
    echo -e "$evidence" >> "$resultfile" 2>&1
  else
    echo "※ U-28 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
    echo " SSH/방화벽에서 특정 IP/대역 제한 정책이 확인되지 않습니다." >> "$resultfile" 2>&1
  fi
...
U_30() {
  ...
  # 기준: 022(8진) = 18(10진)
  local MIN_UMASK_OCT="022"
  local MIN_UMASK_DEC=$((8#$MIN_UMASK_OCT))

  ############################################################
  # 1) systemd 서비스 UMask 점검 (UMask가 명시된 서비스만)
  ############################################################
  ...
  while read -r svc _; do
    ...
    # 8진수 형태만 허용 (예: 0022, 022, 077)
    if ! echo "$umask_val" | grep -qE '^[0-7]{3,4}$'; then
      continue
    fi

    umask_dec=$((8#$umask_val))

    # UMASK 값이 022 이상(= 더 제한적이거나 동일)이어야 양호라는 기존 기준을 그대로 적용
    if [ "$umask_dec" -lt "$MIN_UMASK_DEC" ]; then
      echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
      echo " systemd 서비스 [$svc]에 설정된 UMask 값($umask_val)이 022 미만입니다." >> "$resultfile" 2>&1
      vuln_flag=1
      break
    fi
  done < <(systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1, $2}')

  ############################################################
  # 2) PAM pam_umask.so 적용 여부 점검 (공백/탭/옵션 대응)
  #    - Rocky/RHEL: system-auth/password-auth/postlogin 등
  #    - Debian/Ubuntu: common-session 등
  #    => /etc/pam.d 전체에서 session 라인에 pam_umask.so가 있으면 OK
  ############################################################
  if [ "$vuln_flag" -eq 0 ]; then
    if ! grep -RqsE '^[[:space:]]*session[[:space:]]+.*pam_umask\.so([[:space:]]|$)' /etc/pam.d 2>/dev/null; then
      echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
      echo " PAM 설정에 pam_umask.so 모듈(session)이 적용되어 있지 않습니다." >> "$resultfile" 2>&1
      vuln_flag=1
    fi
  fi

  ############################################################
  # 3) /etc/login.defs UMASK 값 점검
  ############################################################
  if [ "$vuln_flag" -eq 0 ]; then
    local login_umask=""
    login_umask="$(awk '$1=="UMASK"{print $2; exit}' /etc/login.defs 2>/dev/null)"

    if [ -z "$login_umask" ]; then
      echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
      echo " /etc/login.defs 파일에 UMASK 설정이 존재하지 않습니다." >> "$resultfile" 2>&1
      vuln_flag=1
    elif ! echo "$login_umask" | grep -qE '^[0-7]{3,4}$'; then
      echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
      echo " /etc/login.defs UMASK 값 파싱 실패: $login_umask" >> "$resultfile" 2>&1
      vuln_flag=1
    elif [ $((8#$login_umask)) -lt "$MIN_UMASK_DEC" ]; then
      echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
  ...
  if [ "$vuln_flag" -eq 0 ]; then
    local runtime_umask=""
    runtime_umask="$(umask 2>/dev/null || true)"

    if echo "$runtime_umask" | grep -qE '^[0-7]{4}$'; then
      if [ $((8#$runtime_umask)) -lt "$MIN_UMASK_DEC" ]; then
        echo "※ U-30 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1
        echo " 현재 쉘 런타임 umask 값($runtime_umask)이 022 미만입니다." >> "$resultfile" 2>&1
        vuln_flag=1
      fi
    fi
  fi
...
U_32() {
    echo "" >> "$resultfile" 2>&1
    echo "▶ U-32(중) | 2. 파일 및 디렉토리 관리 > 2.19 홈 디렉토리로 지정한 디렉토리의 존재 관리 ◀" >> "$resultfile" 2>&1
    echo " 양호 판단 기준 : 로그인 가능한 계정의 홈 디렉토리가 존재하며, 부적절한 홈 디렉토리 지정이 없는 경우" >> "$resultfile" 2>&1
    ...
    if (( vuln == 0 )); then
        ...
        echo " 로그인 가능한 계정 중 홈 디렉토리가 없거나 존재하지 않는 항목이 있습니다(예: ${missing[0]})." >> "$resultfile" 2>&1
    fi
}
#연수
U_33() {
  ...
  ######################################################################
  # 1) 정상으로 자주 나오는 숨김파일 whitelist (멀티라인로 보기 좋게 유지)
  ######################################################################
...
^/boot/\.vmlinuz-[^/]+\.hmac$|
^/etc/skel/\.(bashrc|bash_profile|bash_logout)$|
^/root/\.(bashrc|bash_profile|bash_logout|cshrc|tcshrc|bash_history)$|
^/home/[^/]+/\.(bashrc|bash_profile|bash_logout|bash_history|lesshst)$
'

  ######################################################################
  # 2) 최근 변경 트랙에서만 제외할 "대량 생성/의미 약한" 구역
  ######################################################################
  local recent_exclude_regex='
^/var/log/|
^/var/cache/|
^/var/lib/(rpm|dnf)/|
^/usr/lib/debug/|
...
'
  ...
  ######################################################################
  # 3) 핵심: 멀티라인 regex -> 1줄 regex로 변환 (줄바꿈 때문에 awk 매칭 깨짐 방지)
  ######################################################################
  ...
  # - "숨김파일 + 실행권한/SetUID/SetGID"는 위치와 무관하게 강하게 의심
  local SUS_STRONG
  SUS_STRONG="$(
    find / \
      "${PRUNE_EXPR[@]}" \
  ...
  # - 숨김파일이 생기면 위험도가 상대적으로 큼: 최근 변경 or 실행/SUID/SGID면 의심
  local SUS_HIGHRISK
  SUS_HIGHRISK="$(
  ...
  # ====== 트랙 C: 일반 위치 + 최근 변경 ======
  # - 최근 변경은 오탐이 많아서 exclude + whitelist 적용
  local SUS_RECENT
  SUS_RECENT="$(
  ...
  if [ "$SUS_COUNT" -gt 0 ]; then
    ...
    echo " [의심 파일 목록]" >> "$resultfile" 2>&1
    echo "$SUS_HIDDEN_FILES" | sed 's/^/  - /' >> "$resultfile" 2>&1

    # 디버깅이 필요할 때만 켜기 (평소엔 주석 권장)
    # echo "[DEBUG] WL_ONELINE=" >> "$resultfile"
    # echo "$WL_ONELINE" | cat -A >> "$resultfile"
    # echo "[DEBUG] EX_ONELINE=" >> "$resultfile"
    # echo "$EX_ONELINE" | cat -A >> "$resultfile"
  else
...
U_37() {
    echo "" >> "$resultfile" 2>&1
    echo "▶ U-37(상) | 3. 서비스 관리 > 3.4 crontab 설정파일 권한 설정 미흡 ◀" >> "$resultfile" 2>&1
    ...
    if [[ -d /var/spool/cron ]]; then
        while IFS= read -r -d '' f; do
            ...
            oct="0$perm"
            if [[ "$owner" != "root" ]]; then
                add_offender "$f (owner=$owner, perm=$perm) : root 소유 권고"
                continue
            fi
            if (( (oct & 63) != 0 )); then
    ...
    if (( vuln == 0 )); then
        ...
        echo " cron 설정 파일/디렉터리에서 소유자 또는 권한(쓰기권한) 이상이 확인되었습니다(예: ${offenders[0]})." >> "$resultfile" 2>&1
    fi
}
...
U_42() {
    ...
    if (( rpc_active == 0 )); then
        ...
        echo " rpcbind(RPC) 서비스가 활성 상태입니다." >> "$resultfile" 2>&1
    fi
}
#연수
U_43() {
...
U_47() {
    ...
    # 2) Sendmail(활성 시) - 자동 판정이 어려워도 기존 로직과 동일하게 양호로 처리(취약 근거 미확인)
    if (( vuln == 0 )) && ( systemctl is-active sendmail.service &>/dev/null || command -v sendmail &>/dev/null ); then
        echo "※ U-47 결과 : 양호(Good)" >> "$resultfile" 2>&1
        return 0
    fi

    # 3) 메일 서비스 미사용(또는 점검 도구 없음)
    if (( vuln == 0 )); then
        echo "※ U-47 결과 : 양호(Good)" >> "$resultfile" 2>&1
        return 0
    fi
...
U_52() {
    ...
    if (( vuln == 0 )); then
        ...
        if (( ${#details[@]} > 0 )); then
            echo " ${details[0]}." >> "$resultfile" 2>&1
        else
            echo " Telnet 활성화 가능 징후가 확인되었습니다." >> "$resultfile" 2>&1
        fi
    fi
}
#연수
...
U_62() {
    echo "" >> "$resultfile" 2>&1
    echo "▶ U-62(하) | 3. 서비스 관리 > 3.29 로그인 시 경고 메시지 설정 ◀" >> "$resultfile" 2>&1
    ...
    if (( ok == 1 )); then
        ...
        echo " 로그인 배너(/etc/issue, /etc/issue.net, SSH Banner)에서 비인가 사용 금지 경고 문구를 확인하지 못했습니다." >> "$resultfile" 2>&1
    fi
}
...
U_66(){
  ...
  echo " 양호 판단 기준 : 필수 로그(messages, secure)가 기록되고 있는 경우" >> "$resultfile" 2>&1
  ...
  # 3. 필수 로그 (취약 판정 기준)
  ########################################
  local CHECK_MSG_FILE=0
  local CHECK_SECURE_FILE=0

  [ -f "/var/log/messages" ] && CHECK_MSG_FILE=1
  [ -f "/var/log/secure" ] && CHECK_SECURE_FILE=1

  # 최근 1일 내 갱신 확인 (로그 실제 기록 여부)
  if [ -f "/var/log/messages" ] && find /var/log/messages -mtime -1 >/dev/null 2>&1; then
    CHECK_MSG_FILE=1
  fi

  if [ -f "/var/log/secure" ] && find /var/log/secure -mtime -1 >/dev/null 2>&1; then
    CHECK_SECURE_FILE=1
  fi

  # 필수 로그 누락 → 취약
  if [ "$CHECK_MSG_FILE" -eq 0 ] || [ "$CHECK_SECURE_FILE" -eq 0 ]; then
    VULN=1
    REASON="필수 로그 누락(messages 또는 secure)"
  fi
  ...
  [ -f "/var/log/maillog" ] && CHECK_MAIL=1
  [ -f "/var/log/cron" ] && CHECK_CRON=1

  echo "$ALL_CONF_CONTENT" | grep -qE "\*\.alert" && CHECK_ALERT=1
  echo "$ALL_CONF_CONTENT" | grep -qE "\*\.emerg" && CHECK_EMERG=1

  [ "$CHECK_MAIL" -eq 0 ] && RECOMMEND="$RECOMMEND maillog"
  [ "$CHECK_CRON" -eq 0 ] && RECOMMEND="$RECOMMEND cron"
  [ "$CHECK_ALERT" -eq 0 ] && RECOMMEND="$RECOMMEND alert"
  [ "$CHECK_EMERG" -eq 0 ] && RECOMMEND="$RECOMMEND emerg"
  ...
  if [ "$VULN" -eq 1 ]; then
    ...
    echo " [근거] $REASON" >> "$resultfile" 2>&1
  else
    echo "※ U-66 결과 : 양호(Good)" >> "$resultfile" 2>&1
    echo " 필수 로그(messages, secure) 정상 동작 확인" >> "$resultfile" 2>&1

    if [ -n "$RECOMMEND" ]; then
      echo " [권장] 다음 로그 설정이 없거나 미확인: $RECOMMEND" >> "$resultfile" 2>&1
    fi
  fi
}
...
U_67() {
  ...
  (
    ...
    # 기본 상한(일반 로그)
    MAX_MODE_DEFAULT="644"

    # 예외 허용(로그인 기록 파일)
    # - wtmp/lastlog: 664까지 허용하는 배포판이 많음
    # - btmp: 660까지 흔함
    # 필요하면 조정 가능
    MAX_MODE_WTMP_LASTLOG="664"
    MAX_MODE_BTMP="660"
    ...
    is_octal_digits() { [[ "${1:-}" =~ ^[0-7]+$ ]]; }

    # 파일별 허용 권한 상한 결정
    max_mode_for_file() {
      local f="$1"
      case "$f" in
        */wtmp|*/lastlog) echo "$MAX_MODE_WTMP_LASTLOG" ;;
        */btmp)           echo "$MAX_MODE_BTMP" ;;
        *)                echo "$MAX_MODE_DEFAULT" ;;
      esac
    }
    ...
    print_line
    echo "점검 항목: $TITLE"
    echo "점검 기준: $LOG_DIR 이하 파일 소유자=root, 권한(기본 ${MAX_MODE_DEFAULT} 이하, wtmp/lastlog ${MAX_MODE_WTMP_LASTLOG} 이하, btmp ${MAX_MODE_BTMP} 이하)"
    print_line
    ...
    while IFS= read -r -d '' f; do
      ...
      # 파일별 허용 상한 적용
      max_mode="$(max_mode_for_file "$f")"

      if is_octal_digits "$mode"; then
        if (( 8#$mode > 8#$max_mode )); then
          bad_reason+=("권한=$mode(허용<=${max_mode})")
        fi
      else
    ...
    if (( vuln > 0 )); then
      ...
      echo "   (wtmp/lastlog/btmp 등 예외 파일은 배포판 정책 확인 후 적용)"
    fi
  ...
  echo "" >> "$resultfile" 2>&1
  echo "▶ U-67(중) | 4. 로그 관리 > 5.3 로그 디렉터리 소유자 및 권한 설정 ◀" >> "$resultfile" 2>&1
  echo " 양호 판단 기준 : /var/log 이하 파일 소유자가 root이고 권한이 정책 기준 이하인 경우(예외: wtmp/lastlog/btmp)" >> "$resultfile" 2>&1

  local _status=""
  _status="$(grep -E '^STATUS=' "$_tmp" 2>/dev/null | tail -n 1 | cut -d= -f2)"

  if [[ "$_status" == "VULN" ]]; then
    echo "※ U-67 결과 : 취약(Vulnerable)" >> "$resultfile" 2>&1

    local _total_cnt _vuln_cnt _err_cnt
    _total_cnt="$(grep -E '점검 파일 수:' "$_tmp" 2>/dev/null | awk -F: '{gsub(/[[:space:]]/,"",$2); print $2}' | tail -n 1)"
    _vuln_cnt="$(grep -E '기준 위반 파일 수:' "$_tmp" 2>/dev/null | awk -F: '{gsub(/[[:space:]]/,"",$2); print $2}' | tail -n 1)"
    _err_cnt="$(grep -E 'stat 조회 실패 파일 수:' "$_tmp" 2>/dev/null | awk -F: '{gsub(/[[:space:]]/,"",$2); print $2}' | tail -n 1)"

    echo " /var/log 이하에서 소유자(root) 및 권한 기준을 충족하지 못한 파일이 존재합니다(위반=${_vuln_cnt:-?}, 점검=${_total_cnt:-?}, stat실패=${_err_cnt:-?})." >> "$resultfile" 2>&1
    echo " [참고] 상세 목록은 내부 로그 출력(기준 위반 파일 목록)을 확인하세요." >> "$resultfile" 2>&1
  elif [[ "$_status" == "GOOD" ]]; then
    echo "※ U-67 결과 : 양호(Good)" >> "$resultfile" 2>&1
  else
    echo "※ U-67 결과 : N/A" >> "$resultfile" 2>&1
    echo " root 권한 필요 또는 점검 불가 상태입니다." >> "$resultfile" 2>&1
  fi
